...
close = 0
signal.signal(signal.SIGTERM,die_gracefully()) #hopefully lets the program close gracefully when "kill PID" is used   
...
while (ser.isOpen() and close == 0):    # breaks the loop when the timer is up
    if timerOn==True:
        curTime = time.time()
